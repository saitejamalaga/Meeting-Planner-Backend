...
let getMeetingDetails = (req, res) => {
    ...
    .exec((err, meetingDetails) => {
        if (err) {
            ...
            logger.info('No Meeting Found', 'Meeting  Controller:getMeetingDetails')
            let apiResponse = response.generate(true, 'No Meeting Found', 404, null)
            res.send(apiResponse)
        } else {
            ...
            MeetingModel.findOneAndRemove({ 'meetingId': req.params.meetingId }).exec((err, result) => {
                if (err) {
                    ...
                    let sendEmailOptions = {
                        ...
                        subject: `Your Meeting Has Been Cancelled: ${newMeetingObj.meetingTopic}`,
                        html: `<h3> Meeting Canceled </h3>
                              <br> Hi , ${newMeetingObj.participantName} .
                              <br> ${newMeetingObj.hostName} cancelled the meeting: ${newMeetingObj.meetingTopic}.
                            `
                    }
                ...
                if (err) {
                    ...
                    let sendEmailOptions = {
                        ...
                        html: `<h3> Your meeting is planned! </h3>
                              <br> Hi , ${newMeetingObj.hostName} has scheduled a meeting via Meeting Planner.
                              <br>  
